...
export default function Expedientes() {
  ...
  const handleVerFichaRapida = async (id) => {
    ...
    try {
      ...
      let sangre = "O+ (Por defecto)";
      let alergias = "Ninguna registrada";
      
      try {
        const expReal = await ApiService.obtenerExpediente(id);
        if (expReal) {
          // Asumiendo que pudieras agregar campos en tu backend, por ahora tomamos los que pusimos o los mock
          sangre = expReal.factorRh || "O+";
          alergias = expReal.alergias || "Ninguna";
        }
      } catch(e) {
        // Silencioso si no tiene expediente
      }
      ...
      setPacienteSeleccionado({
        ...
        tipoSanguineo: sangre, alergias: alergias
      });
    } catch (err) {
      alert("Error al cargar la ficha del paciente desde el servidor");
            ...
            {pacientes.map((p) => (
              <div key={p.id} className="bg-white border border-slate-200 rounded-[32px] p-6 shadow-sm hover:shadow-md transition-all flex flex-col justify-between group animate-in zoom-in-95 duration-300">
                ...
                <div className="mt-6 flex gap-2">
                  ...
                  <button onClick={() => navigate(`/admin/expediente/${p.id}`)} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold p-2.5 rounded-xl transition-colors flex items-center justify-center"><ChevronRight size={16} /></button>
                </div>
              </div>
            ))}
          </div>
